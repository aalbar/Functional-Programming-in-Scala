...
object Huffman {
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def add(chars: List[Char], bit: Bit, accumulated: CodeTable): CodeTable = {
      chars map { c => (c, codeBits(accumulated)(c) :+ bit) }
    }

    def convertToTable(subTree: CodeTree, accumulated: CodeTable): CodeTable = {
      subTree match {
        case node: Leaf => accumulated
        case node: Fork => mergeCodeTables(convertToTable(node.left, add(chars(node.left), 0, accumulated)),
          convertToTable(node.right, add(chars(node.left), 1, accumulated)))
      }
    }

    convertToTable(tree, List[(Char, List[Bit])]())
  }
